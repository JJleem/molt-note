...
pub struct Transcript {
    ...
    /// 전사 모델 식별자.
    pub model: String,
    /// 이 전사 한 건에 **실제로 걸린 시간**(밀리초). 잰 적이 없으면 `None`이다.
    ///
    /// **[`Recording::duration_ms`]와 다른 값이다** — 저쪽은 오디오가 얼마나 긴가이고 이쪽은
    /// 그 오디오를 문장으로 옮기는 데 얼마나 걸렸는가다. 둘은 같은 축에 있지 않으며, 그래서
    /// 하나가 다른 하나를 대신할 수 없다.
    ///
    /// `None`은 **'그때는 재지 않았다'는 정상 상태다** — 이 값을 갖기 전에 저장된 Transcript가
    /// 그렇다. `0`이 아니다: 0을 넣으면 "0초 만에 끝났다"로 읽히고, 그러면 전후 비교가 거짓을
    /// 말한다 (`phase-prompt/05.6` 성공 기준 3).
