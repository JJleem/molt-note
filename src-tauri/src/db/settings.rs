...
pub fn load(connection: &Connection) -> Result<Settings, DatabaseError> {
    ...
    match row {
        Ok((
            ...
            transcription_model,
            // 이 열은 version 9에서 더해졌고 NULL을 허용한다. **NULL은 '아직 고르지 않았다' =
            // 언어를 자동으로 감지한다는 정상 상태다** (ADR-0007 §17.1.4-1) — 여기서 다른 값을
            // 채워 넣지 않는다. 그렇게 하는 순간 고르지 않은 사용자의 DB에 고른 적 없는 언어가
            // 생기고, 그것이 §17.1이 기록한 그 실패다.
            //
            // 저장된 값이 엔진이 아는 언어 코드인지도 묻지 않는다 — `transcription_model`과 같은
            // 규칙이며, 모른다고 해서 저장된 선택을 지우거나 바꾸지 않는다.
            transcription_language,
    ...
    connection
        .execute(
            "INSERT INTO settings (id, recordings_directory, automatic_processing,
                                   ...
                                   notion_parent_page_id, transcription_language)
            ...
            rusqlite::params![
                ...
                // 전사 언어도 자기 열에만 쓰인다. 고르지 않은 값(`None`)은 NULL로 남으며,
                // **여기서 언어를 하나 골라 채워 넣지 않는다** — 저장하는 김에 고른 적 없는
                // 언어가 생기면 그때부터 그것은 사용자가 고른 값처럼 보인다 (ADR-0007 §17.1).
