...
Status:   Accepted (결정) · **구현됨 — 대조는 §15에 있다** (2026-09-04)
          ...
          **2026-09-07에 결정 하나가 더해졌다 (§16)** — 내보낸 파일이 놓인 자리를 여는 수단.
          §4가 세운 위치·파일명·충돌 정책은 **그대로다.**
Date:     2026-09-04 (결정) · 2026-09-04 (§15 구현 대조) ·
          2026-09-07 (§16 — 내보낸 파일의 자리를 여는 결정)
Phase:    Phase 5 — Notion & Markdown Export ·
          Phase 5.6 — Transcription Correctness + Reach (§16)
Task:     TASK-043 (작성) · TASK-054 (§15 구현 대조) · TASK-075 (§16)
Scope:    export 디렉터리·파일명·이름 충돌 정책 · 쓸 Notion API 계약과 헤더 ·
          ...
          **내보낸 파일이 놓인 자리를 여는 수단과 그 허용 범위 (§16 · 2026-09-07 추가)**
```
...
>
> **§16은 §15 뒤에 붙었다 (2026-09-07 · TASK-075).** 2026-09-05의 첫 실사용에서 §4.1이 세운
> 전제 하나 — *"export가 끝나면 전체 경로를 UI가 보여 준다. 사용자가 파일을 찾지 못하는 상태로
> 두지 않는다"* — 가 macOS에서 성립하지 않는 것이 관측됐다. **§4를 고쳐 쓰지 않는다.**
> 그 전제가 언제 왜 부족했는지와 무엇을 더했는지는 §16이 덧붙이는 형태로 적는다.
...
## 16. 나중에 더해진 결정 — 내보낸 파일이 **놓인 자리를 여는 수단** (Phase 5.6 · 2026-09-07)

```text
갱신:  2026-09-07 · TASK-075 (문서 전용 — 소스 · 설정 · 의존성 · 테스트를 바꾸지 않는다)
Phase: 5.6 — Transcription Correctness + Reach (성공 기준 2)
구현:  TASK-071 (2026-09-07)
근거:  2026-09-05 운영자의 첫 실사용 관측 · phase-prompt/05.6 R-4
```

**§4~§15를 고쳐 쓰지 않는다.** export 위치(`exports/`) · 파일명 규칙 · 이름 충돌 정책(§4.3의
번호 붙이기)은 **하나도 바뀌지 않았다.** 여기 적는 것은 §4.1이 마지막 문단에서 세운 전제
하나가 부족했다는 것과, 그 부족을 무엇으로 메웠는가다.

### 16.1 왜 경로를 보여 주는 것만으로 부족했는가 — 2026-09-05 관측

§4.1은 export 위치를 사용자 설정으로 노출하지 않기로 하면서 그 대가를 이렇게 갚기로 했다:

> *"대신 **export가 끝나면 만들어진 파일의 전체 경로를 UI가 보여 준다.** 사용자가 파일을
> 찾지 못하는 상태로 두지 않는다."* (§4.1)

**그 약속은 지켜졌다.** 화면은 만들어진 파일의 전체 경로를 글자로 보여 주고 있었고
(`ExportedFilePayload`의 `path` → Recording Detail의 share 자리), 그렇게 만든 이유까지 주석에
적혀 있었다 [E1].

**그럼에도 2026-09-05에 사람이 그 파일에 도달하지 못했다.**

```text
관측된 사실 (2026-09-05 · phase-prompt/05.6 R-4)

  export 위치            <APP_DATA>/exports/          = ~/Library/Application Support/… 아래
  화면이 보여 준 것       그 파일의 전체 경로 문자열
  사람이 한 일            그 경로로 걸어 들어가려 했다
  일어난 일               도달하지 못했다 — macOS에서 ~/Library 는 Finder 기본 숨김이다
```

**부족했던 것은 정보가 아니라 수단이다.** 경로는 정확했고 화면에 있었다. 그러나 이 플랫폼에서
**경로를 안다는 것과 그 자리에 갈 수 있다는 것이 같지 않았다.**

> **"경로를 보여 주면 찾을 수 있다"는 가정이 이 플랫폼에서 틀렸다.** §4.1이 그 가정 위에서
> 위치를 사용자 설정으로 열지 않기로 했으므로, 가정이 틀린 것을 알았다면 **그 자리에서
> 되갚아야 할 것이 하나 남아 있었던 셈이다.**

**그래서 경로를 없애지 않는다.** 여는 수단은 **대체가 아니라 추가**이며, 전체 경로 문자열은
두 export 자리(Markdown export · Export for AI) 모두에 그대로 남아 있다 [E1 ·
`tests/screen-boundary.test.ts`가 두 자리 모두에서 경로 줄과 여는 수단이 **함께** 있다는 것을
원문으로 확인한다]. 여는 수단이 실패해도 사람은 여전히 경로를 손에 쥐고 있다.

### 16.2 무엇이 그 자리를 여는가

```text
화면 (RecordingDetailScreen)         ← 그리기만 한다. 상태 판정은 src/screens/savedFileView.ts
  └ showSavedFile(path)              src/ipc/commands.ts
      └ show_saved_file              Tauri command 하나 (이름이 늘어난 것은 이것 하나다)
          └ commands/saved_file.rs::SavedFiles::show     ★ 허용을 판정하는 자리
              └ platform/file_manager.rs::FileManager    ★ OS를 부르는 유일한 자리 (INV-10)
                    macOS    open -R
                    Windows  explorer /select,
                    그 밖     FileManagerError::Unsupported → §13의 정의된 실패
```

| 결정 | 내용 | 왜 |
| --- | --- | --- |
| **여는 것은 파일이 아니라 자리다** | 파일을 연결된 앱으로 실행하지 않는다. 파일 관리자에서 **그 파일이 놓인 자리를 드러낼** 뿐이다 | 파일 자체를 실행하면 이 경계가 **사용자가 고른 문서를 임의의 앱에 넘기는 통로**가 된다. 이 Phase가 필요로 한 것은 "방금 만든 것이 어디 있는지 보여 달라"까지다 |
| **OS 지식은 `platform/` 안에만 있다** (INV-10) | `process::Command`도 `cfg(target_os)`도 `file_manager.rs` 밖에 없다 | `platform`의 다른 넷(`app_data_dir` · `clock` · `microphone` · `secret_store`)과 같은 종류의 자리다. 화면 · domain · export 실행 순서는 `open`도 `explorer`도 알지 않으며, `src-tauri/tests/show_saved_file.rs`가 그것을 소스에서 확인한다 [E1] |
| **지원하지 않는 시스템에서 조용히 아무 일도 하지 않지 않는다** | `Unsupported` → §13의 세 질문에 답하는 `Failure` 하나 | 버튼을 눌렀는데 아무 일도 일어나지 않는 상태를 만들지 않는다. `secret_store`가 자격증명 저장소 없는 시스템에서 파일로 조용히 떨어뜨리지 않는 것과 같은 태도다 |
| **새 의존성도 새 권한도 쓰지 않았다** | Tauri 플러그인 없음 · `capabilities/default.json`은 `["core:default"]` 그대로 · `Cargo.toml`에 새 crate 없음 | 표준 라이브러리의 프로세스 실행 하나로 끝난다. **필요하지 않은 권한을 미리 켜 두지 않는다** — 필요해지는 구현으로 바뀌는 날 그 결정은 파일 하나를 고치는 일이다 |
| **export 표면에 넣지 않았다** | 이름은 `show_saved_file`이며 `Exporter`와 자리를 나눴다 | 저쪽은 **파일을 만드는** 일이고 이쪽은 **만들어진 것을 여는** 일이다. 한 이름에 묶으면 `tests/ipc-boundary.test.ts`의 *"export 표면은 파일 하나를 만드는 이름 둘뿐이다"* 가 흐려진다. 그 검사의 `outOfScope`에 `finder` · `explorer`를 더해 **어느 OS의 파일 관리자인지도 이름에 넣지 않는다**는 것을 규칙으로 만들었다 [E1] |

**[E4] 두 명령이 실제로 그 창을 여는지는 확인되지 않았다.** `open -R`도 `explorer /select,`도
이 저장소의 Run에서 실행된 적이 없고, 두 이름과 플래그는 각 플랫폼의 관례를 따른 것이다
(`docs/ADR-0005-microphone-permission.md`가 TCC에 대해 한 것과 같은 방식으로 적어 둔다).
**확인하지 못한 것에 제품을 걸지 않았다** — 실행하지 못하면 그 사실이 정의된 실패로 화면에
도착하고, 전체 경로는 그대로 남는다 (§16.1의 마지막 문단). 실제로 열리는지는
`docs/PHASE-5.6-HUMAN-REVIEW.md` HR-4가 사람에게 묻는다.

### 16.3 왜 임의 경로가 아니라 **앱의 exports 디렉터리 아래로** 제한되는가

**webview가 보낸 경로를 그대로 OS에 넘기지 않는다.** 넘기기 전에 두 가지를 확인하고, 통과하지
못한 요청은 **거절한다** [E1 · `commands/saved_file.rs`].

```text
① 정규화한 결과가 이 앱의 exports_dir() 아래인가      std::fs::canonicalize
② 디렉터리가 아니라 파일 하나인가                      is_file()

두 조건을 함께 통과할 때만 platform::file_manager 에 닿는다 — 그 한 줄을 지나지 않고
OS에 도달하는 경로가 이 crate에 없다.
```

| 왜 이렇게 하는가 | |
| --- | --- |
| **여는 쪽이 범위를 정한다** | 주소를 만드는 쪽(화면)이 아니라 여는 쪽(backend)이 무엇을 열어도 되는지 정한다. **이것은 이 저장소에 이미 있는 태도다** — asset protocol의 scope를 녹음 디렉터리 하나로만 여는 것과 같다 (`crate::run` · `docs/ADR-0006-audio-playback.md` · INV-6) |
| **글자 비교로는 부족하다** | `exports/../../.ssh/id_rsa`는 글자로 보면 `exports/`로 시작하지만 가리키는 자리는 그 밖이다. `canonicalize`는 `..`과 symlink를 실제로 풀어 그 사실을 드러낸다. **글자 비교는 실패 문장을 고르는 데만 쓰이고 허용 판정에는 쓰이지 않는다** |
| **판정은 한 자리에서만 한다** | `file_manager`에는 "어느 파일을 열어도 되는가"에 대한 판정이 **없다.** 두 곳에서 판정하면 한쪽이 느슨해졌을 때 아무도 알아채지 못한다 |
| **거절이 기본값이다** | 앱 데이터 안의 다른 파일(예: `molt-note.db`) · 완전히 밖의 경로 · `..` 탈출 · exports 밖을 가리키는 symlink · 디렉터리 자신 · 빈 문자열 · 더 이상 없는 파일 — **전부 거절된다** [E1 · `src-tauri/tests/show_saved_file.rs`와 모듈 내 테스트가 각각을 값으로 확인한다] |
| **INV-7과 어긋나지 않는다** | 이 통로로 열 수 있는 것은 **사용자가 이미 자기 것으로 아는 markdown 파일**뿐이다. 자격증명은 SQLite에도 exports에도 없다 (§10.5) |

**무엇도 만들지 · 고치지 · 지우지 않는다** (INV-3 · INV-4). 이 모듈에는 저장소에 쓰는 코드도
파일을 쓰거나 지우는 코드도 없다. **디렉터리를 만들지도 않는다** — `ensure_exports_dir()`가
아니라 `exports_dir()`를 쓰므로, 한 번도 내보낸 적 없는 사용자에게 이 경로가 빈 디렉터리를
만들어 두지 않는다. 그때는 *"아직 이 앱이 만든 파일이 없다"* 는 실패다.

**사용자 설정으로 export 위치를 여는 결정은 여전히 하지 않았다** (§4.1 그대로). 이 절이 더한
것은 위치를 고르는 수단이 아니라 **이미 정해진 위치에 도달하는 수단**이다.

### 16.4 이 결정이 틀리면 무엇이 무너지는가

| 틀리는 방식 | 무너지는 범위 | 복구 |
| --- | --- | --- |
| `open -R` · `explorer /select,`가 이 플랫폼에서 그 창을 열지 못한다 | **편의 하나가 사라진다. 기능은 남는다** — 경로 문자열이 그대로 있으므로 사람이 직접 찾아갈 수 있다 | `platform/file_manager.rs` 하나를 고친다. 부르는 자리도 하나다 |
| Tauri 플러그인이나 capability 권한이 실제로 필요했다 | 같은 파일 하나 | 그때 필요한 것만 이름으로 켜고 이유를 적는다 (§16.2의 넷째 줄이 그 자리를 미리 표시해 두었다) |
| exports 아래 제한이 너무 좁다 (예: 녹음 파일 자리도 열고 싶다) | 판정 한 줄 | `SavedFiles::show`의 `starts_with` 한 자리를 넓히는 일이며, **넓히는 순간 무엇을 열 수 있게 되는지가 그 한 줄에서 읽힌다** |

### 16.5 이 갱신이 바꾼 것과 바꾸지 않은 것

**바꾸지 않은 것** — §2의 여덟 결정은 하나도 철회되거나 수정되지 않았다. export 위치도
파일명 규칙도 §4.3의 이름 충돌 정책도 그대로이며, **§4~§15의 어떤 문단도 지우지 않았다.**
Notion 경로는 이 절이 건드리지 않는다.

| 언제 | 무엇을 | 왜 |
| --- | --- | --- |
| 2026-09-07 (TASK-075) | **§16을 추가** — 내보낸 파일이 놓인 자리를 여는 수단 · 그 허용 범위 · 확인하지 못한 것 | §4.1이 세운 전제(*"경로를 보여 주면 찾을 수 있다"*)가 2026-09-05에 macOS에서 성립하지 않는 것이 관측됐다. 구현은 TASK-071이 했고, **그 결정이 어느 문서에도 없었다** |
| 2026-09-07 (TASK-075) | **머리말의 Status · Date · Phase · Task · Scope와 읽기 안내에 이 갱신을 덧붙였다** | 원래 줄을 지우지 않았다 |

**이 Task는 문서만 바꿨다.** `src-tauri/`도 `src/`도 설정도 테스트도 건드리지 않았다.
이 절은 **이미 저장소에 있는 코드를 읽은 결과**이며, 코드와 어긋난다면 코드 쪽이 사실이다.
